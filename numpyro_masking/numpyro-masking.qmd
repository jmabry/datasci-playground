...
From inspection, we can see the parameter estimates for our simple model do not match the ground truth values. Now let's look at the parameters from the model with masking.
...
We see that now the ground truth and estimated parameter values match. The mask handler worked as intended!
